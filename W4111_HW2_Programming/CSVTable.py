...
class CSVTable:
    ...
    def __add_row__(self, row):
        ...
        for index in defined_indexes:
            ...
            if key_string in self.__keys_added__:  # means that key index already exists, row must be added to the list of rows
                self.__indexes__[name][key_string].append(row)  # name 'teamID', key_string 'BOS'
            else:
    ...
    def __get_access_path__(self, fields):
        ...
        # self.__indexes__ example {'Team':{'BOS':[...rows where TeamID = BOS...],'NYE':[...rows where TeamID = NYE...]},
        # 'Year':{'1929':[ ...rows where YearID = 1929...  ], '1958':[...rows where YearID = 1929...],
        # '1975':[...rows where YearID = 1929...]}}
        # ************************ TO DO ***************************
        if self.__indexes__ is None or fields is None or not self.__description__.indexes:
            return None, 0

        count = 0
        most_selective_index = None # default is the first key
        fields_set = set(fields)

        idx_list = self.__description__.indexes  # get index
        for idx in idx_list:
            column_set = set(idx.column_names)
            if column_set.issubset(fields_set):
                curr_count = len(self.__indexes__[idx.index_name])
                if most_selective_index is None or count < curr_count:  # the greater len(self.__indexes__[idx.index_name]), the more selective index_name is.
                    most_selective_index = idx.index_name
                    count = curr_count
        ...
        try:
            if fields is None:  # If there is not project clause, return the base table
                ...
                for r in rows:  # For every row in the table.
                    ...
                    for j in range(0, len(fields)):  # Make a new row with just the requested columns
                        v = r[fields[j]] # r/v is a dictionary.
                        tmp[fields[j]] = v
                    else:
    ...
    def __find_by_template_scan__(self, t, fields=None):
        ...
        # ************************ TO DO ***************************
        if t is None or self.__rows__ is None:
    ...
    def __compute_key__(self, t, index_name):
        res = None
        if self.__description__.indexes is not None:
            for indexdef in self.__description__.indexes:  # search indexdef for index_name
                if indexdef.index_name == index_name:
                    cols = indexdef.column_names
                    vals = []
                    for c in cols:
                        vals.append(t[c])
                    res = "_".join(vals)
    ...
    def __find_by_template_index__(self, t, idx_name, fields=None):
        ...
        """
        # ************************ TO DO ***************************
        keys = self.__compute_key__(t, idx_name)
        res = []
        if keys in self.__indexes__[idx_name]:
            for row in self.__indexes__[idx_name][keys]:
                if self.matches_template(row, t):
                    res.append(row)
            res = self.project(res, fields)
        else:
    ...
    def __find_by_template__(self, template, fields=None, limit=None, offset=None):
        ...
        if indexes is None or indexes == {}:  # also should mean a derived table or some other issue
            ...
            result_index, count = self.__get_access_path__(list(template.keys()))  # change parameter from template to field for get_acess_path
    ...
    def __smart_join__(self, right_r, on_fields, where_template=None, project_fields=None):
        ...
        # Data presentation examples in the following
        # on_fields ["playerID", "yearID"],
        # where_template {"playerID": "smithch01"},
        # project_fields ["playerID", "yearID", "teamID", "AB", "H", "G_all", "G_batting"])

        left_r = self
        left_rows = left_r.__get_row_list__()
        # apply where clause
        if where_template:  # narrow down scope by where clause and use it to find associated rows.
            right_template = right_r.__get_sub_where_template__(where_template)
            right_rows = right_r.__find_by_template__(right_template)
            # apply where_template to scan table only on the intersection of where_template and on_field
            left_template = left_r.__get_sub_where_template__(where_template)
            scan_filter_template = dict()
            for k in on_fields:
                if k in left_template:
                    scan_filter_template[k] = left_template[k]
            if len(scan_filter_template) > 0:
                left_rows = left_r.__find_by_template__(scan_filter_template)
        else:
            right_rows = right_r.__get_row_list__()

        l_index, l_count = left_r.__get_access_path__(on_fields)
        r_index, r_count = right_r.__get_access_path__(on_fields)

        swap = l_count > r_count
        if swap:   # L exchange R, make sure R has the most selective index to be probed quickly.
            ...
            for rr in right_rows:
                if self.matches_template(rr, on_template):
                    if not swap:
                        ...
                        new_r = {**rr, **lr}  # appends two dictionaries together
                    result_rows.append(new_r)
            left_rows_processed += 1
            if left_rows_processed % 10 == 0:
                print("Processed", left_rows_processed, "left rows.")
        ...
        join_result = self.__table_from_rows__("JOIN:" + left_r.__table_name__ + ":" + right_r.__table_name__, result_rows)
        result = join_result.project(result_rows, project_fields)  # projection after join.
    ...
    def __str__(self):
        data = self.__rows__[0:max_rows_to_print]
        header = data[0].keys()
